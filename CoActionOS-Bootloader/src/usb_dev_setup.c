...
void usb_dev_std_setup_stage (void){
	hwpl_usb_rd_ep(USBDEV_PORT, 0x00, (uint8_t *)&usb_setup_pkt);
}
...
void usb_dev_std_datain_stage (void) {
	...
	cnt = hwpl_usb_wr_ep(USBDEV_PORT, 0x80, usb_dev_std_ep0_data.dptr, cnt);
...
void usb_dev_std_dataout_stage (void){
	uint32_t cnt;
	cnt = hwpl_usb_rd_ep(USBDEV_PORT, 0x00, usb_dev_std_ep0_data.dptr);
	usb_dev_std_ep0_data.dptr += cnt;
	usb_dev_std_ep0_data.cnt -= cnt;
}
...
void usb_dev_std_statusin_stage (void){
	hwpl_usb_wr_ep(USBDEV_PORT, 0x80, NULL, 0);
}

void usb_dev_std_statusout_stage (void){
	hwpl_usb_rd_ep(USBDEV_PORT, 0x00, usb_dev_std_ep0_buf);
}
