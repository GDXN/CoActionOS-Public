	...
	case USB_DEV_REQUEST_TO_ENDPOINT:
		...
		if ((usb_dev_std_cfg != 0) && ((i & (USB_LOGIC_EP_NUM-1)) != 0) && (usb_dev_std_ep_mask & j)) {
			if (usb_setup_pkt.wValue.u16 == USB_FEATURE_ENDPOINT_STALL) {
				if (sc) {
					hwpl_usb_stallep(USBDEV_PORT, (void*)i);
					usb_dev_std_ep_halt |=  j;
				} else {
					if ((usb_dev_std_ep_stall & j) != 0) {
						return (true);
					}
					hwpl_usb_unstallep(USBDEV_PORT, (void*)i);
					usb_dev_std_ep_halt &= ~j;
				}
			} else {
				return false;
...
uint32_t usb_dev_std_req_setcfg (void) {
	...
	case USB_DEV_REQUEST_TO_DEVICE:
		if ( usb_setup_pkt.wValue.u8[0] ) {
			...
			while (dptr->bLength) {
					...
					if (((usb_cfg_desc_t *)dptr)->bConfigurationValue == usb_setup_pkt.wValue.u8[0]) {
						...
						for (i = 1; i < USB_LOGIC_EP_NUM; i++) {
							if (usb_dev_std_ep_mask & (1 << i)) {
								hwpl_usb_disableep(USBDEV_PORT, (void*)i);
							}
							if (usb_dev_std_ep_mask & ((1 << USB_LOGIC_EP_NUM) << i)) {
								hwpl_usb_disableep(USBDEV_PORT, (void*)(i|USB_ENDPOINT_IN));
							}
						}
						usb_dev_std_init_ep();
						hwpl_usb_configure(USBDEV_PORT, (void*)true);
				...
				case USB_ENDPOINT_DESCRIPTOR_TYPE:
					if (alternate_setting == 0) {
						...
						hwpl_usb_cfgep(USBDEV_PORT, dptr);
						hwpl_usb_enableep(USBDEV_PORT, (void*)i);
						hwpl_usb_resetep(USBDEV_PORT, (void*)i);
					}
					break;
			...
			usb_dev_std_init_ep();
			hwpl_usb_configure(USBDEV_PORT, (void*)false);
		}
...
static inline uint32_t usb_dev_std_req_setinterface(void);
