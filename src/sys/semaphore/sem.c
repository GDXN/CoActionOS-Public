...
static void priv_sem_wait(void * args);
...
static void priv_sem_post(void * args);


static void priv_sem_trywait(void * args);
typedef struct {
	sem_t * sem;
	struct sched_timeval interval;
} priv_sem_timedwait_t;
static void priv_sem_timedwait(void * args);
...
int sem_post(sem_t *sem){
	...
	if ( new_thread != -1 ){
		hwpl_core_privcall(priv_sem_post, &new_thread);
	}
...
int sem_timedwait(sem_t * sem, const struct timespec * abs_timeout){
	...
	hwpl_core_privcall(priv_sem_timedwait, &args);
...
int sem_trywait(sem_t *sem){
	...
	hwpl_core_privcall(priv_sem_trywait, &args);
...
void priv_sem_wait(void * args){
	...
	sched_table[ task_get_current() ].block_object = args;
...
int sem_wait(sem_t *sem){
	...
	hwpl_core_privcall(priv_sem_wait, sem);
	return 0;
}
