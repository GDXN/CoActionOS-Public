...
#define UART_PORTS HWPL_UART_PORTS
...
static uart_local_t uart_local[UART_PORTS] HWPL_SYS_MEM;

static void read_rx_data(int port);
static void write_tx_data(int port);

LPC_UART_TypeDef * const uart_regs_table[UART_PORTS] = HWPL_UART_REGS;

void _hwpl_uart_dev_power_on(int port){
	if ( uart_local[port].ref_count == 0 ){
		switch(port){
		case 0:
			_hwpl_lpc_core_enable_pwr(PCUART0);
			break;
		case 1:
			_hwpl_lpc_core_enable_pwr(PCUART1);
			break;
		case 2:
			_hwpl_lpc_core_enable_pwr(PCUART2);
			break;
		case 3:
			_hwpl_lpc_core_enable_pwr(PCUART3);
			break;
#ifdef __lpc177x_8x
		case 4:
			_hwpl_lpc_core_enable_pwr(PCUART4);
			break;
#endif
		}
...
void _hwpl_uart_dev_power_off(int port){
	...
	if ( uart_local[port].ref_count > 0 ){
		if ( uart_local[port].ref_count == 1 ){
			_hwpl_core_priv_disable_irq((void*)(UART0_IRQn + port));
			switch(port){
			case 0:
				_hwpl_lpc_core_disable_pwr(PCUART0);
				break;
			case 1:
				_hwpl_lpc_core_disable_pwr(PCUART1);
				break;
			case 2:
				_hwpl_lpc_core_disable_pwr(PCUART2);
				break;
			case 3:
				_hwpl_lpc_core_disable_pwr(PCUART3);
				break;
#ifdef __lpc177x_8x
			case 4:
				_hwpl_lpc_core_disable_pwr(PCUART4);
				break;
#endif
...
int hwpl_uart_setattr(int port, void * ctl){
	...
	if ( ctl_ptr->pin_assign != HWPL_GPIO_CFG_USER ){
		...
		case 1:
			if (1){
				switch(ctl_ptr->pin_assign){
				case 0:
					_hwpl_uart_cfg_pio(1, 0, 16, 15);
					break;
				case 1:
					_hwpl_uart_cfg_pio(1, 2, 1, 0);
					break;
				default:
					errno = EINVAL;
					return -1 - offsetof(uart_attr_t, pin_assign);
		...
		case 2:
			if (1){
				switch(ctl_ptr->pin_assign){
				case 0:
					_hwpl_uart_cfg_pio(2, 0, 11, 10);
					break;
				case 1:
					_hwpl_uart_cfg_pio(2, 2, 9, 8);
					break;
				default:
					errno = EINVAL;
					return -1 - offsetof(uart_attr_t, pin_assign);
		...
		case 3:
			if (1){
				switch(ctl_ptr->pin_assign){
				case 0:
					_hwpl_uart_cfg_pio(3, 0, 1, 0);
					break;
				case 1:
					_hwpl_uart_cfg_pio(3, 0, 26, 25);
					break;
				case 2:
					_hwpl_uart_cfg_pio(3, 4, 29, 28);
					break;
				default:
					errno = EINVAL;
					return -1 - offsetof(uart_attr_t, pin_assign);
	...
	f_div =  ( _hwpl_core_getclock() + 8*baud_rate) / ( baud_rate * 16) ;	// calculate the divisor
	baud_low = f_div & 0xFF;
	...
	_hwpl_core_priv_enable_irq((void*)UART0_IRQn + port);
...
static void exec_writecallback(int port, LPC_UART_TypeDef * uart_regs, void * data){
	...
	if ( uart_local[port].write_callback != NULL ){
		if( uart_local[port].write_callback(uart_local[port].write_context, data) == 0 ){
...
int hwpl_uart_setaction(int port, void * ctl){
	...
	if( action->callback == 0 ){
		...
		uart_local[port].read_callback = 0;
		uart_local[port].write_callback = 0;
...
int hwpl_uart_getall(int port, void * ctl){
	char * dest;
	int i;
	LPC_UART_TypeDef * uart_regs = uart_regs_table[port];
	dest = ctl;
...
void _hwpl_core_uart0_isr(void){
	_hwpl_uart_isr(0);
}

void _hwpl_core_uart1_isr(void){
	_hwpl_uart_isr(1);
}

void _hwpl_core_uart2_isr(void){
	_hwpl_uart_isr(2);
}

void _hwpl_core_uart3_isr(void){
	_hwpl_uart_isr(3);
}
