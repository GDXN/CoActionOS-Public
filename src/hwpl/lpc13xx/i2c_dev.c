...
extern __IO uint32_t * _hwpl_get_iocon_regs(int port, int pin);
...
static i2c_attr_t i2c_local_attr[I2C_PORTS] HWPL_SYS_MEM;
static i2c_local_t i2c_local[I2C_PORTS] HWPL_SYS_MEM;
...
int _hwpl_i2c_dev_port_is_invalid(int port){
...
void _hwpl_i2c_dev_power_on(int port){
	if ( i2c_local[port].ref_count == 0 ){
		_hwpl_lpc_core_enable_pwr(PCI2C);
		...
		_hwpl_core_priv_enable_irq((void*)I2C_IRQn);
...
void _hwpl_i2c_dev_power_off(int port){
	if( i2c_local[port].ref_count > 0 ){
		...
		if( i2c_local[port].ref_count == 0 ){
			LPC_SYSCON->SYSAHBCLKCTRL &= ~(SYSAHBCLKCTRL_I2C);
			_hwpl_core_priv_disable_irq((void*)I2C_IRQn);
			_hwpl_lpc_core_disable_pwr(PCI2C);
		}
	}
}

int _hwpl_i2c_dev_powered_on(int port){
	if ( _hwpl_lpc_core_pwr_enabled(PCI2C) ) return 1;
	return 0;
}
...
int hwpl_i2c_setattr(int port, void * ctl){
	...
	if ( ctl_ptr->pin_assign != HWPL_GPIO_CFG_USER ){
		...
		hwpl_pio_setattr(port, &pattr);

		//set I2C function for pins
		regs_iocon = _hwpl_get_iocon_regs(0, 4);
		*regs_iocon = 0;
		regs_iocon = _hwpl_get_iocon_regs(0, 5);
		*regs_iocon = 0;
