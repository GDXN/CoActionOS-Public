 ...
 */
static inline int circ_buf_rdy(circ_buf_t * cb) HWPL_ALWAYS_INLINE;

/*! \details This checks to see fi the buffer is empty.
 * \return Non-zero if buffer is empty.
 */
static inline int circ_buf_empty(circ_buf_t * cb) HWPL_ALWAYS_INLINE;
...
static inline void circ_buf_put(circ_buf_t * cb, char b) HWPL_ALWAYS_INLINE;
...
static inline char circ_buf8_rdy(circ_buf8_t * cb) HWPL_ALWAYS_INLINE;
