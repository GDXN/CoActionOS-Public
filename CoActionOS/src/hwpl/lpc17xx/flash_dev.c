...
int hwpl_flash_eraseaddr(int port, void * ctl){
	...
	if ( ((uint32_t)ctl < (uint32_t)&_text) ||
		...
		page = get_page(ctl);
		err = hwpl_flash_erasepage(port, (void*)page);
		return err;
	}
	errno = EROFS;
	return -1;
...
int hwpl_flash_writepage(int port, void * ctl){
	...
	int boot_page_addr = get_page_addr(HWPL_LAST_BOOTLOADER_PAGE);
	...
	err = _hwpl_lpc_flash_write_page(get_page((void*)wattr->addr), (void*)wattr->addr, wattr->buf, nbyte);
	if( err < 0 ){
		errno = EIO;
		return -1;
	}
...
int _hwpl_flash_dev_read(const device_cfg_t * cfg, device_transfer_t * rop){
