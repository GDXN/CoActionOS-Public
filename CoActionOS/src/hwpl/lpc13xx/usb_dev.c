...
volatile int usb_dev_test = 0;

typedef struct {
	hwpl_callback_t callback[USB_LOGIC_EP_NUM];
...
int _hwpl_usb_dev_port_is_invalid(int port){
	if ( port != 0 ){
		return 1;
	}
	return 0;
...
int hwpl_usb_setattr(int port, void * ctl){
	...
	_hwpl_core_set_pinsel_func(0, 3, CORE_PERIPH_USB, 0);
...
int hwpl_usb_set_event_handler(int port, void * ctl){
	usb_local.event_handler = (void(*)(usb_spec_event_t,int))ctl;
	return 0;
}
...
int _hwpl_usb_dev_read(const device_cfg_t * cfg, device_transfer_t * rop){
	...
	if ( usb_local.read_ready & (1<<loc) ){
		...
		return hwpl_usb_rd_ep(0, loc, rop->buf);
	} else {
		rop->nbyte = 0;
