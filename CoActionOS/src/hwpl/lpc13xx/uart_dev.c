...
LPC_UART_TypeDef * const uart_regs_table[HWPL_UART_PORTS] = HWPL_UART_REGS;
...
int _hwpl_uart_dev_port_is_invalid(int port){
	if ( (uint32_t)port > HWPL_UART_PORTS ){
		return 1;
	}
	return 0;
}
...
void _hwpl_uart_dev_power_on(int port){
	if( uart_local[port].ref_count == 0 ){
		_hwpl_lpc_core_enable_pwr(PCUART);
		uart_local[port].rx_bufp = NULL;
		uart_local[port].tx_bufp = NULL;
		hwpl_uart_clear(port, NULL);
	}
...
void _hwpl_uart_dev_power_off(int port){
	if( uart_local[port].ref_count > 0 ){
		uart_local[port].ref_count--;
		if( uart_local[port].ref_count == 0 ){
			_hwpl_core_priv_disable_irq((void*)UART_IRQn);
			_hwpl_lpc_core_disable_pwr(PCUART);
		}
	}
}
...
int hwpl_uart_setattr(int port, void * ctl){
	...
	_hwpl_core_priv_enable_irq((void*)UART_IRQn);
