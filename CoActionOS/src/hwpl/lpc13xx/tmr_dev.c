...
LPC_TMR_TypeDef * const tmr_regs_table[HWPL_TMR_PORTS] = HWPL_TMR_REGS;
...
static tmr_local_t _hwpl_tmr_local[HWPL_TMR_PORTS];
...
void _hwpl_tmr_dev_power_on(int port){
	...
	switch(port){
	case 0:
		_hwpl_lpc_core_enable_pwr(PCCT16B0);
		break;
	case 1:
		_hwpl_lpc_core_enable_pwr(PCCT16B1);
		break;
	case 2:
		_hwpl_lpc_core_enable_pwr(PCCT32B0);
		break;
	case 3:
		_hwpl_lpc_core_enable_pwr(PCCT32B1);
		break;
	}

	_hwpl_core_priv_enable_irq((void*)TIMER_16_0_IRQn + port);
}


void _hwpl_tmr_dev_power_off(int port){

	_hwpl_core_priv_disable_irq((void*)TIMER_16_0_IRQn + port);

	switch(port){
	case 0:
		_hwpl_lpc_core_disable_pwr(PCCT16B0);
		break;
	case 1:
		_hwpl_lpc_core_disable_pwr(PCCT16B1);
		break;
	case 2:
		_hwpl_lpc_core_disable_pwr(PCCT32B0);
		break;
	case 3:
		_hwpl_lpc_core_disable_pwr(PCCT32B1);
		break;
	}
}
...
int _hwpl_tmr_dev_powered_on(int port){
	switch(port){
	case 0:
		if ( _hwpl_lpc_core_pwr_enabled(PCCT16B0) ) {
			return 1;
		} else {
			return 0;
		}
		break;
	case 1:
		if ( _hwpl_lpc_core_pwr_enabled(PCCT16B1) ) {
			return 1;
		} else {
			return 0;
		}
		break;
	case 2:
		if ( _hwpl_lpc_core_pwr_enabled(PCCT32B0) ) {
			return 1;
		} else {
			return 0;
		}
		break;
	case 3:
		if ( _hwpl_lpc_core_pwr_enabled(PCCT32B1) ) {
			return 1;
		} else {
			return 0;
		}
...
void input_pin_assign(int port, tmr_attr_t * ctlp){
	...
	case 0:
		if ( ctlp->enabled_ic_chans & (1<<0) ){
			_hwpl_core_set_pinsel_func(0, 2, CORE_PERIPH_TMR, 0);
		}
		break;
	case 1:
		if ( ctlp->enabled_ic_chans & (1<<0) ){
			_hwpl_core_set_pinsel_func(1, 8, CORE_PERIPH_TMR, 1);
		}
		break;
	case 2:
		if ( ctlp->enabled_ic_chans & (1<<0) ){
			_hwpl_core_set_pinsel_func(1, 5, CORE_PERIPH_TMR, 2);
		}
		break;
	case 3:
		if ( ctlp->enabled_ic_chans & (1<<0) ){
			_hwpl_core_set_pinsel_func(1, 0, CORE_PERIPH_TMR, 3);
		}
		break;
	}
}
		...
		if (ctlp->pin_assign == 0){
			...
			case 0:
				if ( ctlp->enabled_oc_chans & (1<<0) ){
					_hwpl_core_set_pinsel_func(0, 8, CORE_PERIPH_TMR, 0);
				}
				if ( ctlp->enabled_oc_chans & (1<<1) ){
					_hwpl_core_set_pinsel_func(0, 9, CORE_PERIPH_TMR, 0);
				}
				break;
			case 1:
				if ( ctlp->enabled_oc_chans & (1<<0) ){
					_hwpl_core_set_pinsel_func(1, 9, CORE_PERIPH_TMR, 1);
				}
				if ( ctlp->enabled_oc_chans & (1<<1) ){
					_hwpl_core_set_pinsel_func(1, 10, CORE_PERIPH_TMR, 1);
				}
				if ( ctlp->enabled_oc_chans & (1<<2) ){
					_hwpl_core_set_pinsel_func(0, 10, CORE_PERIPH_TMR, 1);
				}
				break;
			case 2:
				if ( ctlp->enabled_oc_chans & (1<<0) ){
					_hwpl_core_set_pinsel_func(1, 6, CORE_PERIPH_TMR, 2);
				}
				if ( ctlp->enabled_oc_chans & (1<<1) ){
					_hwpl_core_set_pinsel_func(1, 7, CORE_PERIPH_TMR, 2);
				}
				if ( ctlp->enabled_oc_chans & (1<<2) ){
					_hwpl_core_set_pinsel_func(0, 1, CORE_PERIPH_TMR, 2);
				}
				if ( ctlp->enabled_oc_chans & (1<<3) ){
					_hwpl_core_set_pinsel_func(0, 11, CORE_PERIPH_TMR, 2);
				}
				break;
			case 3:
				if ( ctlp->enabled_oc_chans & (1<<0) ){
					_hwpl_core_set_pinsel_func(1, 1, CORE_PERIPH_TMR, 3);
				}
				if ( ctlp->enabled_oc_chans & (1<<1) ){
					_hwpl_core_set_pinsel_func(1, 2, CORE_PERIPH_TMR, 3);
				}
				if ( ctlp->enabled_oc_chans & (1<<2) ){
					_hwpl_core_set_pinsel_func(1, 3, CORE_PERIPH_TMR, 3);
				}
				if ( ctlp->enabled_oc_chans & (1<<3) ){
					_hwpl_core_set_pinsel_func(1, 4, CORE_PERIPH_TMR, 3);
				}
				break;
			}
