...
#ifndef APPLIB_TMR_HPP_
#define APPLIB_TMR_HPP_

#include <dev/tmr.h>
#include "Pchar.hpp"

/*! \brief Tmr (physcial timer) Class
 * \details This class implements a physical timer.  The Timer class implements
 ...
 *
 */
class Tmr : public Pchar {
public:
	Tmr(port_t port);
	static int getports(){ return HWPL_TMR_PORTS; }

	int open(int flags = 0);
	/*! \brief Get the TMR attributes */
	int getattr(tmr_attr_t * attr);
	...
	/*! \brief Set the TMR action */
	int setaction(tmr_action_channel_t channel /*! The channel to use */,
			int event /*! The event such as Tmr::INTERRUPT */,
			hwpl_callback_t callback /*! Callback executed if Tmr::INTERRUPT is used */ =0,
	...
	int setoc(tmr_action_channel_t channel, uint32_t value){
		tmr_reqattr_t req;
		req.channel = channel;
		req.value = value;
		return setoc(&req);
	...
	int getoc(tmr_action_channel_t channel){
		tmr_reqattr_t req;
		req.channel = channel;
		getoc(&req);
		return req.value;
	...
	int setic(tmr_action_channel_t channel, uint32_t value){
		tmr_reqattr_t req;
		req.channel = channel;
		req.value = value;
		return setic(&req);
	...
	int getic(tmr_action_channel_t channel){
		tmr_reqattr_t req;
		req.channel = channel;
		getic(&req);
		return req.value;
